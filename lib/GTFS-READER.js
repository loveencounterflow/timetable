...
(function() {
  ...
  this.read_trips = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$parse_csv()).pipe(this.$filter_trips(registry)).pipe(P.$sample(ratio, {
      headers: true,
      seed: 5
    })).pipe(this.$clean_trip_record()).pipe(P.$delete_prefix('trip_')).pipe(P.$dasherize_field_names()).pipe(P.$set('%gtfs-type', 'trips')).pipe(P.$rename('id', '%gtfs-id')).pipe(P.$rename('route-id', '%gtfs-routes-id')).pipe(P.$rename('service-id', '%gtfs-service-id')).pipe(this.$register(registry)).pipe(P.$collect_sample(input, 1, function(_, sample) {
  ...
  this.read_stop_times = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(ratio, {
      headers: true
    })).pipe(P.$parse_csv()).pipe(this.$filter_stop_times(registry)).pipe(this.$clean_stop_times_record()).pipe(P.$set('%gtfs-type', 'stop_times')).pipe(P.$dasherize_field_names()).pipe(P.$rename('trip-id', '%gtfs-trip-id')).pipe(P.$rename('stop-id', '%gtfs-stop-id')).pipe(P.$rename('arrival-time', 'arr')).pipe(P.$rename('departure-time', 'dep')).pipe(this.$add_stoptimes_gtfsid()).pipe(this.$register(registry)).pipe(this.$register_stop_id(registry)).pipe(P.$collect_sample(input, 1, function(_, sample) {
  ...
  this.read_stops = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(ratio, {
      headers: true,
      seed: 5
    })).pipe(P.$parse_csv()).pipe(this.$filter_stops(registry)).pipe(this.$clean_stops_record()).pipe(P.$delete_prefix('stop_')).pipe(P.$set('%gtfs-type', 'stops')).pipe(P.$rename('id', '%gtfs-id')).pipe(this.$convert_latlon()).pipe(this.$register(registry)).pipe(P.$collect_sample(input, 1, function(_, sample) {
  ...
  this.main = function(registry, handler) {
    var gtfs_type, limit, message, messages, method, method_name, no_method, no_source, ok_types, route, route_by_types, source_name, t0, tasks, _fn, _i, _j, _k, _len, _len1, _len2, _ref2, _ref3, _ref4, _ref5;
    t0 = 1 * new Date();
    for (source_name in datasource_infos) {
      ...
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        gtfs_type = _ref2[_i];
        if (DEV) {
          if (gtfs_type !== 'stop_times' && gtfs_type !== 'stops') {
