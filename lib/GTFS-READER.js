...
(function() {
  ...
  this.$_XXXX_add_id_indexes = function() {
    ...
    on_data = function(record) {
      var gtfs_id, gtfs_ref_id, gtfs_ref_type, gtfs_type, index_record, match, name, _;
      gtfs_type = "gtfs/" + record['gtfs-type'];
      gtfs_id = record['id'];
      for (name in record) {
        gtfs_ref_id = record[name];
        match = name.match(/^gtfs-(.+?)-id$/);
        if (match == null) {
          continue;
        }
        _ = match[0], gtfs_ref_type = match[1];
  ...
  this.read_routes = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(1 / 10, {
      headers: true,
      seed: 5
    })).pipe(P.$parse_csv()).pipe(this.$clean_routes_record()).pipe(P.$dasherize_field_names()).pipe(P.$set('gtfs-type', 'routes')).pipe(P.$rename('route-id', 'gtfs-id')).pipe(P.$rename('agency-id', 'gtfs-agency-id')).pipe(P.$rename('route-short-name', 'name')).pipe(this.$set_id_from_gtfs_id()).pipe(this.$_XXXX_add_id_indexes()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
