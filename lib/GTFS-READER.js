...
(function() {
  ...
  this.read_stops = function(registry, route, handler) {
    ...
    input.pipe(parser).pipe(T.$skip(global_data_limit)).pipe(T.$as_pods()).pipe(this.$clean_stops_record()).pipe(T.$delete_prefix('stop_')).pipe(T.$set('%gtfs-type', 'stops')).pipe(T.$rename('id', '%gtfs-id')).pipe(this.$convert_latlon()).pipe(this.$register(registry)).pipe(T.$show_sample(input));
