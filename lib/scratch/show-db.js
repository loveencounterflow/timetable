...
(function() {
  var $, $_XXX_show, $filter, $read_trips_from_route, ASYNC, KEY, P, REGISTRY, TRM, alert, badge, debug, echo, help, info, log, new_lte, options, rainbow, rpr, urge, warn, whisper,
    __slice = [].slice;
  ...
  this.test = function(registry, method_name, handler) {
    var prefix, prefixes, tasks, _fn, _i, _len;
    prefixes = ['$.|gtfs/', '$.|gtfs/routes', '%^|gtfs/stop_times'];
    tasks = [];
    _fn = (function(_this) {
      return function(prefix) {
        return tasks.push(function(handler) {
          return registry.createKeyStream({
            gte: prefix,
            lte: KEY.lte_from_gte(prefix)
          }).pipe(P.$count(function(_, count) {
            return help("" + method_name + " " + prefix + "...: " + count + " entries");
          })).on('end', function() {
            return handler(null, null);
          });
        });
    ...
    return ASYNC[method_name](tasks, function(error) {
  ...
  this.main = function(handler) {
    ...
    this.test(registry, 'parallel', handler);
    this.test(registry, 'series', handler);
    return null;
  };
