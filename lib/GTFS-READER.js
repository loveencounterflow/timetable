...
(function() {
  var $, ASYNC, P, REGISTRY, TEXT, TRM, TYPES, alert, badge, datasource_infos, debug, echo, f, global_data_limit, help, info, log, njs_fs, options, rainbow, rpr, urge, warn, whisper, _ref, _ref1;
  ...
  this.read_agency = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(1 / 10, {
      headers: true,
      seed: 5
    })).pipe(P.$parse_csv()).pipe(this.$clean_agency_record()).pipe(P.$delete_prefix('agency_')).pipe(P.$set('%gtfs-type', 'agency')).pipe(P.$rename('id', '%gtfs-id')).pipe(this.$XXXXX_clean_agency_record()).pipe(P.$dasherize_field_names()).pipe(this.$register(registry)).pipe($(f)).pipe(P.$show(input)).on('end', function() {
      info('ok: agency');
      f(null, function(error, record) {
        return whisper('discarded');
      });
      return handler(null);
    });
  ...
  this.$XXXXX_clean_agency_record = function() {
    return $((function(_this) {
      return function(record, handler) {
        var name;
        for (name in record) {
          if (name !== '%gtfs-id' && name !== '%gtfs-type') {
            delete record[name];
          }
        }
