...
(function() {
  ...
  this.read_agency = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$parse_csv()).pipe(this.$clean_agency_record()).pipe(P.$delete_prefix('agency_')).pipe(P.$set('%gtfs-type', 'agency')).pipe(P.$rename('id', '%gtfs-id')).pipe(this.$set_id_from_gtfs_id()).pipe(this.$clean_agency_record()).pipe(P.$dasherize_field_names()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
  ...
  this.read_routes = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(1 / 10, {
      headers: true,
      seed: 5
    })).pipe(P.$parse_csv()).pipe(this.$clean_routes_record()).pipe(P.$dasherize_field_names()).pipe(P.$set('%gtfs-type', 'routes')).pipe(P.$rename('route-id', '%gtfs-id')).pipe(P.$rename('agency-id', '%gtfs-agency-id')).pipe(P.$rename('route-short-name', 'name')).pipe(this.$set_id_from_gtfs_id()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
  ...
  this.read_calendar_dates = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$parse_csv()).pipe(P.$sample(1 / 100, {
      headers: true,
      seed: 5
    })).pipe(this.$clean_calendar_date_record()).pipe(P.$set('%gtfs-type', 'calendar_dates')).pipe(P.$rename('service_id', '%gtfs-id')).pipe(this.$set_id_from_gtfs_id()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
  ...
  this.read_trips = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$parse_csv()).pipe(P.$sample(ratio, {
      headers: true,
      seed: 5
    })).pipe(this.$clean_trip_record()).pipe(P.$delete_prefix('trip_')).pipe(P.$dasherize_field_names()).pipe(P.$set('%gtfs-type', 'trips')).pipe(P.$set('%gtfs-stoptimes', null)).pipe(P.$rename('id', '%gtfs-id')).pipe(P.$rename('route-id', '%gtfs-routes-id')).pipe(P.$rename('service-id', '%gtfs-service-id')).pipe(this.$set_id_from_gtfs_id()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
  ...
  this.read_stop_times = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$sample(ratio, {
      headers: true
    })).pipe(P.$parse_csv()).pipe(this.$clean_stop_times_record()).pipe(P.$set('%gtfs-type', 'stop_times')).pipe(P.$dasherize_field_names()).pipe(P.$rename('trip-id', '%gtfs-trip-id')).pipe(P.$rename('stop-id', '%gtfs-stop-id')).pipe(P.$rename('arrival-time', 'arr')).pipe(P.$rename('departure-time', 'dep')).pipe(this.$add_stoptimes_gtfs_id()).pipe(this.$set_id_from_gtfs_id()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
  ...
  this.read_stops = function(registry, route, handler) {
    ...
    input.pipe(P.$split()).pipe(P.$skip_empty()).pipe(P.$parse_csv()).pipe(this.$clean_stops_record()).pipe(P.$delete_prefix('stop_')).pipe(P.$set('%gtfs-type', 'stops')).pipe(P.$rename('id', '%gtfs-id')).pipe(this.$set_id_from_gtfs_id()).pipe(this.$convert_latlon()).pipe(REGISTRY.$register(registry)).pipe(P.$collect_sample(1, function(_, sample) {
