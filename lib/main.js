...
(function() {
  var ASYNC, T, TEXT, TRM, TYPES, alert, as_transformer, badge, create_readstream, datasource_infos, debug, echo, help, info, log, new_parser, njs_fs, options, rainbow, rpr, urge, warn, whisper, _new_parser;
  ...
  this.read_agencies = function(route, registry, handler) {
    ...
    input.pipe(parser).pipe(T.$skip(3)).pipe(T.$as_pods()).pipe(this.$clean_agency_record()).pipe(this.$fix_ids()).pipe(T.$delete_prefix('agency_')).pipe(T.$add_n4j_system_properties('node', 'agency')).pipe(T.$rename('id', '%gtfs-id')).pipe(this.$add_agency_id()).pipe(T.$dasherize_field_names()).pipe(T.$register(registry)).pipe(T.$show_sample(input));
  ...
  this.read_stop_times = function(route, registry, handler) {
    ...
    input.pipe(parser).pipe(T.$skip(3)).pipe(T.$as_pods()).pipe(this.$clean_stoptime_record()).pipe(this.$fix_ids()).pipe(T.$delete_prefix('trip_')).pipe(T.$add_n4j_system_properties('node', 'stoptime')).pipe(T.$dasherize_field_names()).pipe(T.$rename('id', '%gtfs-trip-id')).pipe(T.$rename('stop-id', '%gtfs-stop-id')).pipe(this.$add_stoptime_idx()).pipe(this.$add_stoptime_id()).pipe(T.$register(registry)).pipe(T.$show_sample(input));
  ...
  this.read_stations = function(route, registry, handler) {
    ...
    input.pipe(parser).pipe(T.$skip(3)).pipe(T.$as_pods()).pipe(this.$clean_station_record()).pipe(this.$fix_ids()).pipe(T.$delete_prefix('stop_')).pipe(T.$copy('name', '%gtfs-name')).pipe(this.$normalize_station_name()).pipe(this.$add_station_system_name()).pipe(T.$rename('id', '%gtfs-id')).pipe(T.$add_n4j_system_properties('node', 'station')).pipe(this.$convert_latlon()).pipe(this.$add_station_id(registry)).pipe(T.$register(registry)).pipe(T.$show_sample(input));
  ...
  this.read_trips = function(route, registry, handler) {
    ...
    input.pipe(parser).pipe(T.$skip(3)).pipe(T.$as_pods()).pipe(this.$clean_trip_record()).pipe(this.$fix_ids()).pipe(T.$delete_prefix('trip_')).pipe(T.$dasherize_field_names()).pipe(T.$rename('id', '%gtfs-id')).pipe(T.$rename('route-id', '%gtfs-route-id')).pipe(T.$rename('service-id', '%gtfs-service-id')).pipe(T.$copy('headsign', '%gtfs-headsign')).pipe(this.$normalize_headsign()).pipe(this.$add_headsign_system_name()).pipe(T.$add_n4j_system_properties('node', 'trip')).pipe(this.$add_trip_id(registry)).pipe(T.$register(registry)).pipe(T.$show_sample(input));
    whisper('reading GTFS trips...');
    return null;
  };

  this.read = function(handler) {
    var gtfs_type, message, messages, method, no_method, no_source, ok_types, registry, route, route_by_types, source_name, tasks, _fn, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    registry = {};
